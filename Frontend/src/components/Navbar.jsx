    ...
    <>
      ...
      <nav className="fixed top-0 left-0 w-full z-20 bg-white shadow-md flex justify-between sm:px-5 sm:py-2 px-3 items-center">
        ...
        <div className="sm:hidden relative">
          <button onClick={() => setHamburger((s) => !s)} className="focus:outline-none transition-transform duration-300" type="button">
            {hamburger ? <IoClose size={35} /> : <IoMenu size={35} />}
          </button>

          {/* Dropdown */}
          <div
            className={`absolute right-0 mt-4 min-w-64 bg-white shadow-lg rounded-lg p-4 flex flex-col gap-3 text-lg font-semibold z-50 transform transition-all duration-300 origin-top ${
