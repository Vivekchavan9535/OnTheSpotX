...
export default function EditServiceModal({ isOpen, onOpenChange }) {
	...
	const formik = useFormik({
		...
		onSubmit: async (values, { setSubmitting, resetForm }) => {
			if (!editId) return;
			await dispatch(
				updateService({ editId, formData: values, resetForm })
			);
		},
	});
	...
	return (
		<Dialog open={isOpen && !!serviceToEdit} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-[425px]">
				<DialogHeader>
					<DialogTitle>Edit Service: {serviceToEdit?.title}</DialogTitle>
				</DialogHeader>
				...
				<form onSubmit={formik.handleSubmit} className="space-y-4 pt-4">
					...
					<div className="flex justify-end gap-2 pt-4">
						<DialogClose asChild>
							<Button type="button" variant="outline" onClick={() => dispatch(clearEditId())}>
								Cancel
							</Button>
						</DialogClose>
